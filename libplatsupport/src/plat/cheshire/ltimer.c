 ...
 */
/* Implementation of a logical timer for cheshire SoC platform.
 *
 */
...
static int create_ltimer(ltimer_t *ltimer, ps_io_ops_t ops)
{
    ...
    int error = ps_calloc(&ops.malloc_ops, 1, sizeof(cheshire_ltimer_t), &ltimer->data);
    if (error) {
        return error;
    }
...
{
    assert(ltimer != NULL);
    cheshire_ltimer_t *timer = ltimer->data;
